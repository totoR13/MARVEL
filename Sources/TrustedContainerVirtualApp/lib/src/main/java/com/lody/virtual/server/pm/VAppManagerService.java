...
import com.lody.virtual.custom.Checker;
...
public class VAppManagerService implements IAppManager {
    ...
    public synchronized InstallResult installPackage(String path, int flags, boolean notify) {
        ...
        // TODO: in this function I can check package and other properties of the target plugin app to be installed
        //  -> if these checks do not pass, the plugin app will be discard
        //      -> in this way, the new threat model is not applicable because the trusted container app checks before
        //      -> Moreover, the container app hooks the child class loader, but, in case of malicious  this do not have the
        ...
        Checker.checkApkSignature(pkg.packageName, path);
        Checker.checkClassesDexBytes(pkg.packageName, path);
